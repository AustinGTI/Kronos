 ...
 * @param {Date} date - a string representing a date in the format DD/MM/YYYY
...
/**
 * A function to extract the date_key from a session id by removing the last 4 digits and converting the rest to a date then to a string
 * @param {number} session_id - the session id to extract the date key from
 */
